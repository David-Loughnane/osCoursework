...
void sem_wait (int id, short unsigned int num) {
  struct sembuf op[] = {
    // if value of semaphore non-negative decrements by 1-> allocate resource
    // if the semaphore variable is now negative, process executing wait is blocked
    //   added to the semaphore queue
    // SEM_UNDO flag records changes made to semaphore, kernel undos all changes on exit
    {num, -1/*, SEM_UNDO*/}
  };
  semop (id, op, 1);
}

//to avoid starvation a semaphore has an associated queue of process (usually FIFO)
